...
exports.svgfit = {
  ...
  default_options: function(test) {
    ...
    actual = grunt.file.read('tmp/expand2/search.svg');
    expected = grunt.file.read('test/expected/expand2/search.svg');
    test.equal(actual, expected, 'expand2 - svg should fit exact bounds');

    actual = grunt.file.read('tmp/compact/search.svg');
    expected = grunt.file.read('test/expected/compact/search.svg');
    test.equal(actual, expected, 'compact - svg should fit exact bounds');

    actual = grunt.file.read('tmp/fileArray/search.svg');
    expected = grunt.file.read('test/expected/fileArray/search.svg');
    test.equal(actual, expected, 'fileArray - svg should fit exact bounds');

    actual = grunt.file.read('tmp/fileObject/test.svg');
    expected = grunt.file.read('test/expected/fileObject/test.svg');
  ...
  allFilesProcessed: function(test) {
    ...
    actual = grunt.file.expand({cwd: 'tmp/expand2' }, '*').length;
    expected = grunt.file.expand({cwd: 'test/expected/expand2' }, '*').length;
    test.equal(actual, expected, 'expand2 - should be same number of output files as input');

    actual = grunt.file.expand({cwd: 'tmp/compact' }, '*').length;
    expected = grunt.file.expand({cwd: 'test/expected/compact' }, '*').length;
    test.equal(actual, expected, 'compact - should be same number of output files as input');

    actual = grunt.file.expand({cwd: 'tmp/fileArray' }, '*').length;
    expected = grunt.file.expand({cwd: 'test/expected/fileArray' }, '*').length;
    test.equal(actual, expected, 'fileArray - should be same number of output files as input');

    actual = grunt.file.expand({cwd: 'tmp/fileObject' }, '*').length;
    expected = grunt.file.expand({cwd: 'test/expected/fileObject' }, '*').length;
